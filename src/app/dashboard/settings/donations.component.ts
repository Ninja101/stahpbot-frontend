...
@Component({
	selector: 'sb-settings-donations',
	templateUrl: '../views/settings/donations.html',
	animations: [
		trigger( 'btnState', [
...
export class SettingsDonationsComponent implements AfterViewInit
{
	public settings: any = { };
	public original: any = { };
	...
	public twitchAlertsLoading: boolean = false;
	public twitchAlertsClientId: string = "w7P1GqagOMd3VP3S25u4rR5gpYHThzMU16OgfxhY";
	public twitchAlertsScope: string[] = [ 'donations.read' ];
	public twitchAlertsRedirectUrl: string = "http://localhost:8080/auth/twitchalerts";
	...
	)
	{
		...
		breadcrumbService.addFriendlyNameForRoute( '/settings/donations', 'Donations' );
		pagevarsService.setPageTitle( "Donation Settings" );
		titleService.setTitle( "Donation Settings - Stahpbot" );
	...
	connectTwitchAlerts( )
	{
		this.twitchAlertsLoading = true;
		window.location.href = sprintf(
			'https://streamlabs.com/api/v1.0/authorize?response_type=code&client_id=%s&redirect_uri=%s&scope=%s',
			this.twitchAlertsClientId,
			encodeURIComponent( this.twitchAlertsRedirectUrl ),
			this.twitchAlertsScope.join( '+' )
		);
	}
}